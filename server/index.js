...
app.post('/api/db/import', upload.single('dbfile'), async (req, res) => {
  ...
  try {
    ...
    if (data.departments?.length) for (const d of data.departments) await db.exec('INSERT INTO departments VALUES ($1,$2,$3,$4)', [d.id, d.label, d.icon, d.color]);
    if (data.articles?.length) for (const a of data.articles) await db.exec('INSERT INTO articles VALUES ($1,$2,$3,$4,$5)', [a.id, a.dept_id, a.descrizione, a.qtynuovo, a.qtyrigenerato]);
    if (data.history?.length) for (const h of data.history) await db.exec('INSERT INTO history (descrizione, date, qty, customer, origin, tipo, operation) VALUES ($1,$2,$3,$4,$5,$6,$7)', [h.descrizione, h.date, h.qty, h.customer, h.origin, h.tipo, h.operation]);
    io.emit('state_sync', await db.getState());
