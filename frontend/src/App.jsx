...
export default function App() {
  ...
  const goBack = () => {
    if (view === 'warehouse' || view === 'agenda' || view === 'settings_global') setView('main');
    else setView('warehouse');
  };
  ...
  // ✅ FIX 3: Gestione della transazione e della cancellazione articoli perfettamente allineata
  const confirmTx = () => {
    // Se la spunta di eliminazione è attiva, esegui subito ed esci (evita crash di validazione)
    if (delArtConfirm && modal.type === 'realignment') {
      console.log("🔴 FRONTEND: Tentativo eliminazione", modal.articleId, modal.deptId); 
    ...
    <div className="min-h-screen bg-gray-900 font-sans text-gray-100 select-none">
      ...
      <header className="sticky top-0 z-20 flex items-center justify-between px-4 py-3 bg-gray-900/95 backdrop-blur border-b border-gray-800">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setView('settings_global')} className="p-2 rounded-lg hover:bg-gray-800 transition" title="Configurazione">
        ...
        <main className="flex flex-col h-[calc(100vh-56px)] p-4 pb-4 items-center justify-center">
          ...
          <div className="w-full max-w-md space-y-4">
            <button onClick={async () => { 
              ...
              }} className="w-full py-4 bg-gray-700 text-gray-200 rounded-xl flex gap-3 items-center justify-center hover:bg-gray-600 transition text-lg font-medium">
              <Database className="w-6 h-6"/> Backup Database
            </button>
      ...
      {view === 'main' && (
        <main className="flex flex-col items-center justify-center h-[calc(100vh-56px)] gap-6 p-6">
          <button onClick={() => setView('warehouse')} className="w-full max-w-md p-8 bg-gray-800 rounded-2xl border border-gray-700 hover:border-blue-500 transition flex flex-col items-center gap-3 active:scale-95">
            <Package className="w-12 h-12 text-blue-400"/><span className="text-2xl font-bold">Magazzino</span><span className="text-gray-400 text-sm">Gestisci reparti e articoli</span>
          </button>
          <button onClick={() => setView('agenda')} className="w-full max-w-md p-8 bg-gray-800 rounded-2xl border border-gray-700 hover:border-purple-500 transition flex flex-col items-center gap-3 active:scale-95">
            <Calendar className="w-12 h-12 text-purple-400"/><span className="text-2xl font-bold">Agenda</span><span className="text-gray-400 text-sm">Gestisci appuntamenti operatori</span>
          </button>
        </main>
      )}
      ...
      {view === 'settings' && <main className="flex flex-col h-[calc(100vh-56px)] p-4 pb-4"><h2 className="text-xl font-bold mb-4">Gestione Reparti</h2><div className="flex-1 space-y-3 mb-4 overflow-y-auto">{data.departments.map(d => (<div key={d.id} className="flex justify-between p-4 bg-gray-800 rounded-xl border border-gray-700"><div className="flex gap-3 items-center"><div className={`w-10 h-10 ${d.color} rounded-lg flex items-center justify-center`}><d.icon className="w-5 h-5"/></div><span>{d.label}</span></div><div className="flex gap-2"><button onClick={() => setEditDeptModal({ isOpen: true, deptId: d.id, currentLabel: d.label, newLabel: d.label })} className="text-blue-400 p-2 hover:text-blue-300 transition hover:bg-blue-900/30 rounded-lg"><Settings className="w-5 h-5"/></button><button onClick={() => setDelDeptModal({ isOpen: true, deptId: d.id, label: d.label })} className="text-red-400 p-2 hover:text-red-300 transition hover:bg-red-900/30 rounded-lg"><Trash2 className="w-5 h-5"/></button></div></div>))}</div><div className="space-y-3">
        ...
        <button onClick={() => setAddDeptModal({ isOpen: true, name: '' })} className="w-full py-4 bg-emerald-600 text-white rounded-xl flex gap-2 items-center justify-center"><Plus className="w-5 h-5"/>Nuovo Reparto</button></div></main>}
        ...
        <div className="flex flex-col h-[calc(100vh-56px)] bg-gray-900 overflow-hidden">
          <div className="bg-gray-800 p-4 border-b border-gray-700 z-10 shadow-lg flex-shrink-0" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
            ...
            <div key={currentMonth.toISOString()} className="grid grid-cols-7 gap-1.5 month-transition">
              {days.map((d, i) => {
                ...
                let dayClass = !d.date ? 'opacity-10 border-transparent' :
                               isSelected ? 'bg-blue-600/40 border-blue-500 text-white ring-2 ring-blue-400/50' :
                               isToday ? 'bg-cyan-500/20 border-cyan-500/60 text-cyan-300 font-semibold' :
                               'bg-gray-800/50 border-gray-700 text-gray-300 hover:bg-gray-700/40';
                if (!isSelected && d.isWeekend) dayClass += ' text-red-400';

                return (
                  <button key={i} onClick={() => d.date && setSelectedDate(d.date)} disabled={!d.date} className={`h-12 w-full rounded-lg border flex flex-col items-center justify-center relative transition ${dayClass}`}>
                    {d.day}
                    {hasAppt(d.date) && <Star className="w-3 h-3 text-yellow-400 absolute top-1 right-1"/>}
                  </button>
                );
              })}
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 pb-20">
            {appointments.filter(a => a.date === selectedDate).sort((a,b) => a.time.localeCompare(b.time)).map(appt => (
              <div key={appt.id} className="bg-gray-800 rounded-xl border border-gray-700 p-4 flex items-center gap-4">
                ...
                <div className="flex gap-1">
                  {(appt.operator === 'A' || appt.operator === 'AC') && <span className="px-2 py-1 rounded text-xs font-bold bg-red-500/20 text-red-400 border border-red-500/30">A</span>}
                  {(appt.operator === 'C' || appt.operator === 'AC') && <span className="px-2 py-1 rounded text-xs font-bold bg-green-500/20 text-green-400 border border-green-500/30">C</span>}
                </div>
            ...
            {appointments.filter(a => a.date === selectedDate).length === 0 && (
              <div className="text-center text-gray-500 mt-10"><Calendar className="w-12 h-12 mx-auto mb-2 opacity-50"/>Nessun appuntamento per questa data</div>
            )}
          </div>
