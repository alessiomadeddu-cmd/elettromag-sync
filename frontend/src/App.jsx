...
export default function App() {
  ...
  const connect = (key) => {
    // In dev: si connette a localhost:3000. In prod: usa window.location.origin
...
const confirmTx = () => {
  ...
  // ✅ Usa array separati per nomi icone e colori (stringhe pure)
  const ICON_NAMES = ['Radio', 'Zap', 'Sun', 'Shield', 'Package', 'Wrench', 'Lightbulb', 'Server', 'Battery', 'Plug'];
  const nd = { 
    id: `d${Date.now()}`, 
    label: addDeptModal.name.trim(), 
    iconName: ICON_NAMES[idx % ICON_NAMES.length], // ✅ Stringa sicura
...
data.history.map(h => { 
  const op = h.operation || h.op || ''; // ✅ Legge correttamente la colonna DB
  const c = h.tipo==='Nuovo'?'text-green-400':h.tipo==='Rigenerato'?'text-yellow-400':'text-blue-400'; 
  const q = op === 'unload' ? `-${h.qty}` : `+${h.qty}`; // ✅ Meno per scarico, più per carico
  const d = h.customer?`Cli: ${h.customer}`:h.origin?`Org: ${h.origin}`:'-'; 
 // ✅ Nuova logica: Verde tenue per Carico, Rosso tenue per Scarico
