...
hist.push({ 
  desc: modal.descrizione, date: now, qty: q, 
  customer: modal.customer.trim(), origin: modal.origin.trim(), 
  tipo: modal.targetType === 'N' ? 'Nuovo' : 'Rigenerato', 
  op: modal.type 
});
}
sock.current.emit('confirm_tx', {
type: modal.type, artId: modal.articleId,
newN: parseInt(modal.newQtyN), newR: parseInt(modal.newQtyR),
...
return (
 <main className="flex flex-col h-[calc(100vh-56px)] p-4 pb-24" style={{ paddingBottom: 'max(6rem, env(safe-area-inset-bottom))' }}>
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 bg-black/60 backdrop-blur p-4" onClick={e => e.target === e.currentTarget && closeModal()}>
      <div className="w-full max-w-md bg-gray-800 rounded-2xl border border-gray-700 shadow-2xl max-h-[80vh] overflow-y-auto p-5" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between mb-4">
          {/* ✅ FIX 2: Mostra descrizione articolo accanto al tipo operazione */}
          <h3 className="font-bold">
            {modal.type==='load'?`📥 Carico - ${modal.descrizione}`:modal.type==='unload'?`📤 Scarico - ${modal.descrizione}`:'🔄 Riallineamento'}
          </h3>
        ...
        <div className="flex gap-3 mt-4">
          ...
          {/* ✅ FIX 1: Chiude la modale dopo l'eliminazione */}
          <button onClick={() => {
