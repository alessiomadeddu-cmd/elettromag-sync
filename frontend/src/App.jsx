...
const ICON_POOL = Object.values(ICONS);
...
export default function App() {
  ...
  const [delArtConfirm, setDelArtConfirm] = useState(false); // ✅ Fix 2: Stato per cancellazione articolo
  ...
  const connect = (key) => {
    ...
    s.on('state_sync', (st) => {
      ...
      setData({
        ...st,
        articles: arts,
        departments: st.departments.map(d => ({ ...d, icon: ICONS[d.icon] || Package }))
      });
    });
  };

  // ⏱️ Long Press Articoli (5s)
  ...
  const openModal = (d, a, t) => {
    ...
    if (!art) return;
    setDelArtConfirm(false); // ✅ Reset checkbox articolo
  ...
  const DeptView = () => {
    ...
    return (
      <main className="flex flex-col h-[calc(100vh-56px)] p-4 pb-24" style={{ paddingBottom: 'max(6rem, env(safe-area-inset-bottom))' }}> {/* ✅ Fix 3: Safe Area iOS */}
  ...
  return (
    <div className="min-h-screen bg-gray-900 font-sans text-gray-100 select-none">
      {/* ✅ Fix 3: Safe Area iOS Header */}
      <header className="sticky top-0 z-20 flex items-center justify-between px-4 py-3 bg-gray-900/95 backdrop-blur border-b border-gray-800" style={{ paddingTop: 'max(12px, env(safe-area-inset-top))' }}>
      ...
      {view === 'home' && <main className="p-4 pb-24" style={{ paddingBottom: 'max(6rem, env(safe-area-inset-bottom))' }}><h1 className="text-xl font-bold mb-6">Reparti</h1><div className="grid grid-cols-2 gap-4">{data.departments.map(d => (<button key={d.id} onClick={() => { setSelectedDept(d); setView('dept'); }} className={`${d.color} flex flex-col items-center justify-center gap-3 p-6 rounded-2xl shadow-lg text-white font-semibold text-lg active:scale-95 min-h-[140px]`}><d.icon className="w-10 h-10"/><span>{d.label}</span></button>))}</div></main>}
      {view === 'dept' && <DeptView/>}
      
      {view === 'history' && <main className="flex flex-col h-[calc(100vh-56px)] p-4 pb-4" style={{ paddingBottom: 'max(6rem, env(safe-area-inset-bottom))' }}><h2 className="text-xl font-bold mb-3 flex gap-2 items-center"><BookOpen className="w-5 h-5"/>Storico</h2><div className="flex-1 bg-gray-800 rounded-xl border border-gray-700 overflow-hidden flex flex-col"><div className="sticky top-0 grid grid-cols-12 gap-2 px-4 py-3 bg-gray-900 border-b text-xs font-bold text-gray-400 uppercase"><div className="col-span-3">Data</div><div className="col-span-3">Descrizione</div><div className="col-span-3 text-center">Qtà</div><div className="col-span-3">Dest/Orig</div></div><div className="flex-1 overflow-y-auto">{data.history.length===0?<div className="p-8 text-center text-gray-400">Nessun movimento</div>:data.history.map(h => {const op = h.operation || h.op || '';const c = h.tipo==='Nuovo'?'text-green-400':h.tipo==='Rigenerato'?'text-yellow-400':'text-blue-400';const q = op === 'unload' ? `-${h.qty}` : `+${h.qty}`;const d = h.customer? `Cli: ${h.customer}` :h.origin? `Org: ${h.origin}` :'-';const bg = op === 'load' ? 'bg-green-900/20' : 'bg-red-900/35';return (<div key={h.id} className={`grid grid-cols-12 gap-2 items-center px-4 py-3 border-b border-gray-700/50 hover:bg-gray-600/20 text-sm ${bg}`}><div className="col-span-3 text-gray-400 truncate">{h.date}</div><div className="col-span-3 text-gray-200 truncate">{h.descrizione}</div><div className={`col-span-3 text-center font-semibold tabular-nums ${c}`}>{q}</div><div className="col-span-3 text-gray-300 truncate">{d}</div></div>);})}</div></div></main>}
      
      {view === 'settings' && <main className="flex flex-col h-[calc(100vh-56px)] p-4 pb-4" style={{ paddingBottom: 'max(6rem, env(safe-area-inset-bottom))' }}><h2 className="text-xl font-bold mb-4">Impostazioni</h2><div className="flex-1 space-y-3 mb-4 overflow-y-auto">{data.departments.map(d => (<div key={d.id} className="flex justify-between p-4 bg-gray-800 rounded-xl border border-gray-700"><div className="flex gap-3 items-center"><div className={`w-10 h-10 ${d.color} rounded-lg flex items-center justify-center`}><d.icon className="w-5 h-5"/></div><span>{d.label}</span></div><button onClick={() => setDelDeptModal({ isOpen: true, deptId: d.id, label: d.label })} className="text-red-400 p-2"><Trash2 className="w-5 h-5"/></button></div>))}</div><div className="space-y-3"><button onClick={() => { localStorage.clear(); window.location.reload(); }} className="w-full py-4 bg-red-900/50 text-red-200 rounded-xl flex gap-2 items-center justify-center"><WifiOff className="w-5 h-5"/>Disconnetti / Reset</button><button onClick={() => setAddDeptModal({ isOpen: true, name: '' })} className="w-full py-4 bg-emerald-600 text-white rounded-xl flex gap-2 items-center justify-center"><Plus className="w-5 h-5"/>Nuovo Reparto</button></div></main>}
      ...
      {modal.isOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 bg-black/60 backdrop-blur p-4" onClick={e => e.target === e.currentTarget && closeModal()}>
          <div className="w-full max-w-md bg-gray-800 rounded-2xl border border-gray-700 shadow-2xl max-h-[80vh] overflow-y-auto p-5" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between mb-4"><h3 className="font-bold">{modal.type==='load'?'📥 Carico':modal.type==='unload'?'📤 Scarico':'🔄 Riallineamento'}</h3><button onClick={closeModal}><X/></button></div>
            {modal.type==='realignment'?(
              <>
                ...
                {/* ✅ Fix 2: Checkbox elimina articolo */}
            ...
            <div className="flex gap-3 mt-4">
              ...
              {/* ✅ Fix 2: Logica pulsante dinamico */}
              <button onClick={() => {
      ...
      {/* ✅ Fix 1: Modale eliminazione reparto blindata */}
      {delDeptModal.isOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 bg-black/60 backdrop-blur p-4" onClick={e => {
          if (e.target === e.currentTarget) { setDelDeptModal({isOpen:false,deptId:'',label:''}); setDelOk(false); setHoldProg(0); }
        }}>
